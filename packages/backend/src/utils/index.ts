...
export const startZombienet = async (
  dirName: string,
  fileName: string,
  networkName: string,
  confFile: string,
  ...
  try {
    ...
    if (fs.existsSync(binDirectory)) {
      ...
      await addIntoNetworksDirectory(fileName, confFile, networkName);
      await runZombienet(dirName, fileName, networkName, ZOMBIENET_VERSION);
      await manageNetworkJson(dirName, fileName, networkName, confFile, dslFileName, dslFile);
    }
  } catch (error) {
    console.error(error);
  }
